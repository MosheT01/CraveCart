...
3. **Fly machines must import the same bytes.** Re-run **`deploy-fly-ci.sh`** or **`deploy-fly.ps1 -FromGcpSecretManager`**. **`deploy-fly-ci.sh`** runs **`GET https://<FLY_APP>.fly.dev/health`** with a Bearer copied from GCP Secret Manager and, on failure, prints **HTTP status + the first chunk of the response body** (`cat -v`) so each run yields **facts** (403 Unauthorized vs HTML vs timeouts), not guesses. **`SKIP_FLY_BEARER_HEALTH=1`** in the Fly Actions step env skips that check entirely if you need deploy green while debugging.
...
The workflow [`.github/workflows/deploy-main.yml`](../.github/workflows/deploy-main.yml) runs when **`main`** is pushed (and can be **[Run workflow](https://docs.github.com/en/actions/using-workflows/manually-running-a-workflow)** manually). It submits **[`cloudbuild.yaml`](../cloudbuild.yaml)** from the repo checkout, then syncs Kroger MCP to Fly using **[`kroger-mcp/deploy-fly-ci.sh`](../kroger-mcp/deploy-fly-ci.sh)** (`gcloud`/Secret Manager → **`fly secrets import`** without **`--stage`**, then **`fly deploy`**). Non-staged import restarts Machines with vault env immediately; **`--stage`** followed by **`fly deploy --remote-only`** has left some apps serving **`/health`** **403** on Bearer checks (staged secrets not yet on the instances taking traffic). Order vs Cloud Build is intentional: **`cravecart-web`** must reach the **`sync-public-urls`** revision first so Kroger redirects come from **`gcloud run services describe cravecart-web`** before Fly publishes **`KROGER_REDIRECT_URI`**.
...
- **Fly**: [`deploy-fly-ci.sh`](../kroger-mcp/deploy-fly-ci.sh) publishes secrets via **stdin** to **`fly secrets import`**, then **`fly deploy`**; local **[`deploy-fly.ps1`](../kroger-mcp/deploy-fly.ps1)** uses **`fly secrets set`** then **`fly deploy`** (no **`--stage`** in CI — see workflow section above). Stdin import avoids values in **`ps`**/`argv`; still treat tokens as sensitive (**`FLY_API_TOKEN`** masking keeps logs clean).
