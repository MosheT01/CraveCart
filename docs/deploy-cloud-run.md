...
3. **Fly machines must import the same bytes.** Re-run **`deploy-fly-ci.sh`** or **`deploy-fly.ps1 -FromGcpSecretManager`**. **`deploy-fly-ci.sh`** probes **`GET https://<FLY_APP>.fly.dev/health`** with Bearer ( **`FLY_APP`** from **`FLY_APP_NAME`** ), not **`EXTERNAL_KROGER_SIDECAR_URL`**—so Actions can catch Fly ↔ GCP mismatch even when the Cloud Build–side URL typo would have failed the probe against the wrong hostname.
