...
1. **Timing / `:latest` drift (common in CI).** Cloud Build deploys **`cravecart-web`** first; the Fly step runs minutes later. If **`INTERNAL_SIDECAR_SECRET`** gets a **new Secret Manager version** in between, **`cravecart-web`** may still expose the payload from the revision it was deployed with, while **`fly secrets import`** reads **`:latest`** and ends up with **different bytes**—often showing up **right after** a green deploy. **GitHub Actions** [`.github/workflows/deploy-main.yml`](../.github/workflows/deploy-main.yml) resolves **one numeric version** once per run and passes **`_INTERNAL_SIDECAR_SECRET_VERSION`** into [`cloudbuild.yaml`](../cloudbuild.yaml) **and** **`INTERNAL_SIDECAR_SECRET_VERSION`** into [`kroger-mcp/deploy-fly-ci.sh`](../kroger-mcp/deploy-fly-ci.sh). Local **`gcloud builds submit`** without that substitution still uses substitution default **`latest`** (acceptable for ad-hoc builds).
