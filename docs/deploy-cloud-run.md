...
1. **Timing / `:latest` drift (common in CI).** Cloud Build deploys **`cravecart-web`** first; the Fly step runs minutes later. If **`INTERNAL_SIDECAR_SECRET`** gets a **new Secret Manager version** in between, **`cravecart-web`** may still expose the payload from the revision it was deployed with, while **`fly secrets import`** reads **`:latest`** and ends up with **different bytes**—often showing up **right after** a green deploy. **GitHub Actions** [`.github/workflows/deploy-main.yml`](../.github/workflows/deploy-main.yml) runs [`scripts/ci/pin_internal_sidecar_secret_version.py`](../scripts/ci/pin_internal_sidecar_secret_version.py), which resolves the numeric revision via **`access:latest`** (needs only **`secretmanager.versions.access`**, included in **`roles/secretmanager.secretAccessor`** — avoids **`permission denied`** on **`secrets.versions.describe`**, which needs **`secretmanager.versions.get`**). It passes **`_INTERNAL_SIDECAR_SECRET_VERSION`** into [`cloudbuild.yaml`](../cloudbuild.yaml) **and** **`INTERNAL_SIDECAR_SECRET_VERSION`** into [`kroger-mcp/deploy-fly-ci.sh`](../kroger-mcp/deploy-fly-ci.sh). Local **`gcloud builds submit`** without that substitution still uses substitution default **`latest`** (acceptable for ad-hoc builds).
2. **Web service must mount the secret.** `cloudbuild.yaml` **`deploy-web`** uses **`--set-secrets … INTERNAL_SIDECAR_SECRET=INTERNAL_SIDECAR_SECRET:<version>`** (CI passes a concrete version; default is **`latest`**). Confirm the Cloud Run **runtime** SA has **`secretmanager.secretAccessor`** on **`INTERNAL_SIDECAR_SECRET`** and redeploy **`cravecart-web`** after adding the mapping.
3. **Fly machines must import the same bytes.** Re-run **`deploy-fly-ci.sh`** or **`deploy-fly.ps1 -FromGcpSecretManager`**. **`deploy-fly-ci.sh`** verifies **`GET BASE_URL/health`** with Bearer returns **200** when **`EXTERNAL_KROGER_SIDECAR_URL`** is set (GitHub Actions passes it), so a bad sync **fails CI** instead of breaking chat in production.
...
The workflow [`.github/workflows/deploy-main.yml`](../.github/workflows/deploy-main.yml) runs when **`main`** is pushed (and can be **[Run workflow](https://docs.github.com/en/actions/using-workflows/manually-running-a-workflow)** manually). It submits **[`cloudbuild.yaml`](../cloudbuild.yaml)** from the repo checkout, then syncs Kroger MCP to Fly using **[`kroger-mcp/deploy-fly-ci.sh`](../kroger-mcp/deploy-fly-ci.sh)** (`gcloud`/Secret Manager → **`fly secrets import --stage`** → **`fly deploy`**). On Fly Machines, **`--stage`** updates encrypted secrets **without** rolling VMs; **`fly deploy`** applies them so you get **one** release instead of import-triggered rollout + deploy. Order vs Cloud Build is intentional: **`cravecart-web`** must reach the **`sync-public-urls`** revision first so Kroger redirects come from **`gcloud run services describe cravecart-web`** before Fly publishes **`KROGER_REDIRECT_URI`**.
...
| **Fly Kroger MCP** | [`kroger-mcp/deploy-fly-ci.sh`](../kroger-mcp/deploy-fly-ci.sh) runs **`gcloud secrets versions access`** for **`KROGER_CLIENT_*`**, **`INTERNAL_SIDECAR_SECRET`**, optionally **`KROGER_LOCATION_ID`**, then **`fly secrets import --stage`** and **`fly deploy`** so staged secrets ride the same release as the image. **`INTERNAL_SIDECAR_SECRET`** must match **`cravecart-web`** Secret Manager (**version pinning** in CI — see troubleshooting above). |
...
- **Fly**: [`deploy-fly-ci.sh`](../kroger-mcp/deploy-fly-ci.sh) publishes secrets via **stdin** to **`fly secrets import --stage`**, then **`fly deploy`**; local **[`deploy-fly.ps1`](../kroger-mcp/deploy-fly.ps1)** uses **`fly secrets set --stage`** the same way. Stdin import avoids values in **`ps`**/`argv`; still treat tokens as sensitive (**`FLY_API_TOKEN`** masking keeps logs clean).
